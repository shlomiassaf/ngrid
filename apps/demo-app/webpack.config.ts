      ...
      use: [
        {
          ...
          options: {
            context: process.cwd(),
            transpileOnly: true,
            configFile: 'apps/demo-app/tsconfig.server.service-worker.json',
          }
        }
      ]
    },
...
function updateWebpackConfig(webpackConfig: Configuration): Configuration {
  ...
  const plugins = ServiceWorkerTsPlugin.create(
    ...
    {
      entry: Path.join(process.cwd(), 'libs/demo-apps/shared/src/lib/datastore/datasource.store-service-worker.ts')
    }
  );
