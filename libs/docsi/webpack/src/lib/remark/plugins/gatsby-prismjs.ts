...
export function gatsbyRemarkPrismJs() {
  ...
  return markdownAST => {
    ...
    for (const codeNode of codeNodes) {
      const htmlCode = RE_CODE.test(codeNode.value) ? codeNode.value.replace(/\{/g, `{{ '{' }}`) : codeNode.value;
      codeNode.value = `<div code-viewer>${htmlCode}</div>`;
    }
  };
}
