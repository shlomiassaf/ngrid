// Commit: 79e4d283e6f646b8b0b4740967679f7adc312081
...
export class ItemSizeAverager {
  ...
  protected _averageItemSize: number;

  /** The default size to use for items when no data is available. */
  protected _defaultItemSize: number;
...
export class AutoSizeVirtualScrollStrategy implements VirtualScrollStrategy {
  ...
  protected _viewport: CdkVirtualScrollViewport | null = null;
  ...
  protected _averager: ItemSizeAverager;

  /** The last measured scroll offset of the viewport. */
  protected _lastScrollOffset: number;

  /** The last measured size of the rendered content in the viewport. */
  protected _lastRenderedContentSize: number;
  ...
  protected _updateRenderedContentAfterScroll() {
  ...
  protected _checkRenderedContentOffset() {
    const viewport = this._viewport!;
   ...
   * scroll offset.
   */
  protected _renderContentForCurrentOffset() {
    const viewport = this._viewport!;
    const scrollOffset = viewport.measureScrollOffset();
    this._lastScrollOffset = scrollOffset;
    this._removalFailures = 0;
   ...
   */
  protected _getVisibleRangeForIndex(startIndex: number): ListRange {
    const viewport = this._viewport!;
   ...
   */
  protected _expandRange(range: ListRange, expandStart: number, expandEnd: number): ListRange {
    const viewport = this._viewport!;
  ...
  protected _updateTotalContentSize(renderedContentSize: number) {
    const viewport = this._viewport!;
    const renderedRange = viewport.getRenderedRange();
